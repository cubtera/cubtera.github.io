---
title: Quick start
description: Quick start guide for Cubtera
sidebar:
...
&emsp; Cubtera is cli tool for multi-layer and multi-dimension infrastructures management. It allows you to manage your infrastructure with different dimensions, and run your units with different dimensions values from single inventory.
Let's start with quick start guide to get you up and running with Cubtera and understand the core concepts.

## Core Concepts

&emsp; Cubtera is designed to help you manage your infrastructure with different dimensions, allowing you to separate your infrastructure by different environments, regions, accounts, resource types, etc. The core concepts we've achieved with Cubtera are:
- **Dimension types**: Logical groupings or layers of your infrastructure.
- **Dimension entities**: Specific instances or configurations within a dimension.
- **Units**: The smallest operational `cubic` in Cubtera, representing specific scripts or actions that can be performed on your infrastructure.
- **Runners**: Implementations that define how `units` are executed (e.g., Terraform, Bash, Helm, etc.).


### Dimension types

&emsp; Cubtera uses the concept of "dimensions" to organize and manage infrastructure. Dimensions can represent any logical grouping or layer of your infrastructure. Examples include:
...
&emsp; Dimensions could be hierarchical and can have parent-child relationships, as well as flat dimensions with no relationships.

### Dimension entities

&emsp; Each dimension type can have multiple `entities` with specific values of that type. `Dim entities` are defined in JSON files and stored in the inventory grouped by dimension type:
...
&emsp; Examples of dimension entities include see in the next **`Units`** section.

### Units

&emsp; Units are the smallest operational `cubics` in Cubtera, representing specific scripts or actions that can be performed on your infrastructure by different tools like Terraform, Opentofu, Bash Scripts, Helm, etc.

&emsp; Units are stored in the inventory and can be run with different dimensions values and using cubtera variables defined in the inventory and named following simple convention.
...
Cubtera is a Rust-based application designed to manage and orchestrate infrastructure deployments using IaC. It provides a flexible architecture for executing various runner implementations, managing data sources, handling unit manifests, and offering a comprehensive CLI for interacting with different functionalities.
...
```rust title="path/to/src/core/runner/mod.rs"
pub trait Runner {
...
```rust title="path/to/src/core/runner/tf/mod.rs"
...
For further details and contributions, refer to the [Cubtera Repository](https://github.com/cubtera/cubtera).
